...
unsigned int next_prime(unsigned int num)
{
    ...
    while (__primes[i] < num)
        i++;
    if (i >= sizeof(__primes) / sizeof(unsigned int))
        return num;
    return __primes[i];
}
...
htable_t *make_htable(unsigned int capacity, hsize_callback sizefn, hcmp_callback cmpfn)
{
    int i;
...
static unsigned int __str_size_fn(void *buf)
{
